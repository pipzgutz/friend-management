...
public interface FriendConnectionDao extends CrudRepository<FriendConnection, Long> {
    ...
    @Query("SELECT fc FROM FriendConnection fc INNER JOIN fc.friend f WHERE fc.friendConnectedTo.email = :email AND fc.isBlocked = false")
    List<FriendConnection> findByFriendConnectionEmailNotBlocked(@Param("email") String email);
